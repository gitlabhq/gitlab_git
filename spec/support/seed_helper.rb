module SeedHelper
  ...
  def create_bare_seeds
    ...
    system(git_env, *%W(git clone -q --bare #{GITHUB_URL}), chdir: SUPPORT_PATH)
  end

  def create_normal_seeds
    FileUtils.mkdir_p(SUPPORT_PATH)
    system(git_env, *%W(git clone -q #{TEST_REPO_PATH} #{TEST_NORMAL_REPO_PATH}))
  end

  def create_mutable_seeds
    FileUtils.mkdir_p(SUPPORT_PATH)
    system(git_env, *%W(git clone -q #{TEST_REPO_PATH} #{TEST_MUTABLE_REPO_PATH}))
    system(git_env, *%w(git branch -q -t feature origin/feature),
